...
def save_video(final_frames, output_path, fps=30, bitrate="5000k", codec="libx264"):
    ...
    clip = ImageSequenceClip(list(final_frames), fps=fps)
    # Write the video file with specified options
    clip.write_videofile(
        output_path,
        codec=codec,
        bitrate=bitrate,
        preset='slow',  # 'slow' for better quality (and larger file size), 'ultrafast' for faster rendering
        ffmpeg_params=['-crf', '10']  # CRF (Constant Rate Factor) value ranges from 0 (lossless) to 51 (worst quality); lower is better
    )
