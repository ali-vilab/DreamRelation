...
T5_MODEL = 'pretrain_models_ckpt/t5-v1_1-xxl'
...
class DitModelFactory(ModelFactory):
    ...
    def get_model(
        ...
        *,
        ...
        fast_init=True,
        lora_target=None,
        load_custom_lora=False,
        **dreamrelation_kwargs,
    ):
        separate_qkv_x = model_kwargs.get('separate_qkv_x', False) if model_kwargs else False
        if separate_qkv_x:
            from genmo.mochi_preview.dit.joint_model.asymm_models_joint_dreamrelation import AsymmDiTJoint
        else:
        ...
        if load_custom_lora and lora_target is None:
            lora_target = ["q", "k", "v"]
        ...
        if lora_path is not None:
            if lora_path.endswith(".safetensors"):
                ...
                with safe_open(lora_path, framework="pt") as f:
                    if load_custom_lora:
                        for k in f.keys():
                            if any(lora_pos == k.rsplit('.', 1)[-1] for lora_pos in lora_target):
                    ...
                    if load_custom_lora and 'qkv_proj_lora_target' in lora_kwargs:
                        lora_kwargs['qkv_proj_lora_target'] = lora_target
                    print(f"Loaded LoRA kwargs: {lora_kwargs}")
            else:
                lora = load_to_cpu(lora_path, weights_only=False)
                if "state_dict" in lora:
            ...
            model_kwargs.update(dreamrelation_kwargs)
        ...
        if local_rank == 0 and load_checkpoint:
            ...
            if separate_qkv_x:
                replace_qkv_with_separate_layers(model, model_kwargs)

            if lora_sd:
                mismatch = model.load_state_dict(lora_sd, strict=strict_load) # type: ignore
                # print("Keys in model not matched: {}".format(mismatch[0]))
                print("Keys in lora checkpoint not matched: {}".format(mismatch[1]))
...
def sample_model(device, dit, conditioning, **args):
    ...
    def model_fn(*, z, sigma, cfg_scale):
        if cond_batched:
            ...
            if "forward_lora_name" in args:
                cond_text["forward_lora_name"] = args["forward_lora_name"]
                cond_null["forward_lora_name"] = args["forward_lora_name"]
            with torch.autocast("cuda", dtype=torch.bfloat16):
...
class MochiSingleGPUPipeline:
    def __init__(
        ...
        **dreamrelation_kwargs,
        ...
        with t("load_dit"):
            self.dit = dit_factory.get_model(local_rank=0, device_id=init_id, world_size=1, fast_init=fast_init, strict_load=strict_load, **dreamrelation_kwargs) # type: ignore
        with t("load_vae"):
...
def cast_dit(model, dtype):
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            assert any(
                n in name for n in ["mlp", "t5", "mod_", "attn.qkv_", "attn.proj_", "final_layer", "attn.to_q_", "attn.to_k_", "attn.to_v_", "projector"]
...
def replace_qkv_with_separate_layers(model, model_kwargs):
    from genmo.mochi_preview.dit.joint_model.asymm_models_joint_dreamrelation import AsymmetricAttention
    if "q_lora_names" in model_kwargs or "k_lora_names" in model_kwargs or "v_lora_names" in model_kwargs:
        from genmo.mochi_preview.dit.joint_model.lora_dreamrelation import LoraLinear
    else:
        from genmo.mochi_preview.dit.joint_model.lora import LoraLinear
    for name, module in model.named_modules():
        if isinstance(module, AsymmetricAttention):
            qkv_layer = getattr(module, 'qkv_x', None)
            if qkv_layer is None:
                print(f"Module {name} does not have a qkv_x attribute.")
                continue
            
            original_weight = qkv_layer.weight.data.clone()

            in_features = qkv_layer.in_features
            out_features = qkv_layer.out_features // 3

            qkv_lora_kwargs = dict(
                bias=module.qkv_bias,
                device=None,
                r=model_kwargs['qkv_proj_lora_rank'],
                lora_alpha=model_kwargs['qkv_proj_lora_alpha'],
                lora_dropout=model_kwargs['qkv_proj_lora_dropout'],
                # lora_target=qkv_proj_lora_target,
            )
            if 'test_lora_names' in model_kwargs:
                qkv_lora_kwargs['test_lora_names'] = model_kwargs['test_lora_names']
            q_lora_kwargs = dict(qkv_lora_kwargs)
            k_lora_kwargs = dict(qkv_lora_kwargs)
            v_lora_kwargs = dict(qkv_lora_kwargs)
            q_lora_kwargs['lora_names'] = model_kwargs.get('q_lora_names', [])
            k_lora_kwargs['lora_names'] = model_kwargs.get('k_lora_names', [])
            v_lora_kwargs['lora_names'] = model_kwargs.get('v_lora_names', [])

            module.to_q_x = LoraLinear(in_features, out_features, **q_lora_kwargs)
            module.to_k_x = LoraLinear(in_features, out_features, **k_lora_kwargs)
            module.to_v_x = LoraLinear(in_features, out_features, **v_lora_kwargs)

            q_weight, k_weight, v_weight = original_weight.split(out_features, dim=0)
            module.to_q_x.weight.data = module.to_q_x.weight.data.to(dtype=q_weight.dtype)
            module.to_q_x.weight.data.copy_(q_weight)
            module.to_k_x.weight.data = module.to_k_x.weight.data.to(dtype=k_weight.dtype)
            module.to_k_x.weight.data.copy_(k_weight)
            module.to_v_x.weight.data = module.to_v_x.weight.data.to(dtype=v_weight.dtype)
            module.to_v_x.weight.data.copy_(v_weight)

            delattr(module, 'qkv_x')

            print(f"Replaced qkv_x in {name} with to_q_x, to_k_x, to_v_x.")

            if model_kwargs.get('separate_qkv_y', False):
                qkv_layer = getattr(module, 'qkv_y', None)
                if qkv_layer is None:
                    print(f"Module {name} does not have a qkv_y attribute.")
                    continue
                ...
                qkv_lora_kwargs = dict(
                    ...
                    lora_dropout=model_kwargs['qkv_proj_lora_dropout'],
                    # lora_target=qkv_proj_lora_target,
                    test_lora_names=model_kwargs['test_lora_names']
                )
                q_lora_kwargs = dict(qkv_lora_kwargs)
                k_lora_kwargs = dict(qkv_lora_kwargs)
                v_lora_kwargs = dict(qkv_lora_kwargs)
                ...
                module.to_q_y = LoraLinear(in_features, out_features, **q_lora_kwargs)
                module.to_k_y = LoraLinear(in_features, out_features, **k_lora_kwargs)
                module.to_v_y = LoraLinear(in_features, out_features, **v_lora_kwargs)

                q_weight, k_weight, v_weight = original_weight.split(out_features, dim=0)
                module.to_q_y.weight.data = module.to_q_y.weight.data.to(dtype=q_weight.dtype)
                module.to_q_y.weight.data.copy_(q_weight)
                module.to_k_y.weight.data = module.to_k_y.weight.data.to(dtype=k_weight.dtype)
                module.to_k_y.weight.data.copy_(k_weight)
                module.to_v_y.weight.data = module.to_v_y.weight.data.to(dtype=v_weight.dtype)
                module.to_v_y.weight.data.copy_(v_weight)

                delattr(module, 'qkv_y')

                print(f"Replaced qkv_y in {name} with to_q_y, to_k_y, to_v_y.")
