...
class AsymmetricAttention(nn.Module):
    def __init__(
        ...
        separate_qkv_x: bool = False,
        separate_qkv_y: bool = False,
        **attention_kwargs,
    ):
        super().__init__()
        ...
        self.separate_qkv_x = separate_qkv_x
        self.separate_qkv_y = separate_qkv_y

        # Input layers.
        self.qkv_bias = qkv_bias
        if not separate_qkv_x:
    ...
    def run_qkv_y(self, y, **kwargs):
        ...
        if cp.is_cp_active():
            ...
            if self.separate_qkv_y:
                q_y = self.to_q_y(y, **kwargs)  # (B, L, dim_y)
                q_y = q_y.view(q_y.size(0), q_y.size(1), local_heads, self.head_dim)
                k_y = self.to_k_y(y, **kwargs)  # (B, L, dim_y)
                k_y = k_y.view(k_y.size(0), k_y.size(1), local_heads, self.head_dim)
                v_y = self.to_v_y(y, **kwargs)  # (B, L, dim_y)
                v_y = v_y.view(v_y.size(0), v_y.size(1), local_heads, self.head_dim)
            else:
        ...
        q_y = self.q_norm_y(q_y)  # 1, text_seqlen, 24, 128
        k_y = self.k_norm_y(k_y)  # 1, text_seqlen, 24, 128
        return q_y, k_y, v_y
    ...
    def prepare_qkv(
        ...
        **kwargs,
        ...
        if not self.separate_qkv_x:
            ...
            input_to_q_x = x
            q_x = self.to_q_x(input_to_q_x, **kwargs)  # (B, M, dim_x)
            assert q_x.dtype == torch.bfloat16
            q_x = cp.all_to_all_collect_tokens_q_k_v(q_x, self.num_heads)  # (B, N, local_h, head_dim)

            input_to_k_x = x
            k_x = self.to_k_x(input_to_k_x, **kwargs)
            assert k_x.dtype == torch.bfloat16
            k_x = cp.all_to_all_collect_tokens_q_k_v(k_x, self.num_heads)  # (B, N, local_h, head_dim)

            input_to_v_x = x
            v_x = self.to_v_x(input_to_v_x, **kwargs)
            assert v_x.dtype == torch.bfloat16
            v_x = cp.all_to_all_collect_tokens_q_k_v(v_x, self.num_heads)  # (B, N, local_h, head_dim)
        q_x = self.q_norm_x(q_x)  # 1, 17490, 24, 128
        q_x = apply_rotary_emb_qk_real(q_x, rope_cos, rope_sin)
        k_x = self.k_norm_x(k_x)  # 1, 17490, 24, 128
        ...
        if B == 1:
            ...
            if text_seqlen > 0:
                y = y[:, :text_seqlen]  # Remove padding tokens.  1, text_seqlen, 1536
                y = modulated_rmsnorm(y, scale_y)  # (B, L, dim_y)  1, text_seqlen, 1536
                q_y, k_y, v_y = self.run_qkv_y(y, **kwargs)  # (B, L, local_heads, head_dim)
    ...
    def forward(
        ...
        *,
        ...
        forward_lora_name: Optional[list] = None,
        **rope_rotation,
        ...
        """
        B, L, _ = y.shape  # 1, 256, 1536
        _, M, _ = x.shape  # 1, 17490, 3072
        ...
        # if self.after_attn_decompose_frequency:
        #     pass
...
class AsymmDiTJoint(nn.Module):
    ...
    def prepare(
        ...
        y_feat = self.t5_yproj(t5_feat)  # (B, L, t5_feat_dim) --> (B, L, D)  1, 256, 1536
