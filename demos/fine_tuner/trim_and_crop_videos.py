...
def truncate_videos(folder, output_folder, duration, resolution):
    ...
    for file_path in tqdm(video_files):
        try:
            ...
            if video.w == video.h:
                click.echo(
                    f"Resizing {file_path} (square) and padding to meet target resolution {resolution}."
                )
                scaling_ratio = target_height / float(video.h)
                new_width = int(video.w * scaling_ratio)
                video = video.resize((new_width, target_height))

                # Calculate padding needed to achieve target width
                padding = (target_width - new_width) // 2
                video = video.margin(
                    left=padding, right=padding, color=(0, 0, 0)
                )
            elif target_height > video.h:
                click.echo(
                    f"Resizing {file_path} to meet the target resolution {resolution} by adjusting height."
                )
                scaling_ratio = target_height / float(video.h)
                new_width = int(video.w * scaling_ratio)
                video = video.resize((new_width, target_height))
