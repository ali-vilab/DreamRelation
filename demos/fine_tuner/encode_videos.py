...
def preprocess(ctx: GPUContext, vid_path: Path, shape: str, reconstruct: bool):
    ...
    if video.shape[2] != H or video.shape[3] != W:
        print(f"Resizing {vid_path} from {video.shape[2]}x{video.shape[3]} to {H}x{W}")
        # Resize each frame in the video
        video = torch.stack([resize(frame, [H, W]) for frame in video.permute(1, 0, 2, 3)])
